...
#
# This module is a verbatim, byte-faithful copy of the reference
# fractional-indexing algorithm — the official Python port
# `httpie/fractional-indexing-python` of `rocicorp/fractional-indexing`
# (the Figma / David-Greenspan scheme). It is third-party code carried
# source-only inside `livespec_runtime` (which has no vendoring
# machinery and is itself copied source-only into every consumer's
# `_vendor/` tree); it is therefore EXCLUDED from this repo's
# first-party lint / type / coverage gates (see pyproject.toml). The
# livespec-facing surface is the keyword-only wrapper in `rank.py`;
# import `generate_key_between` / `generate_n_keys_between` /
# `validate_order_key` from here only through that wrapper.
#
#   Upstream:  https://github.com/httpie/fractional-indexing-python
#   Commit:    a4f9e2d334c0d51b743b34012d2d1f35465e96e9 (2024-04-19)
#   Licence:   CC0 1.0 Universal (public domain dedication)
#   See NOTICES at the repo root for the attribution entry.
#
"""
