...
secrets present → `Success` carrying a typed `GithubAppConfig` (with the
optional installation-id pin + API-URL override carried through); any
absent-or-empty required var → `Failure` carrying a `GithubAppAuthError`
that names EVERY missing var, points at the tenant's credential_wrapper,
and states there is NO fleet fallback.

The boundary is on the `Result` railway (not `IOResult`): it reads an
injected `environ` snapshot and performs no IO, so it is a total,
process-free function of its inputs. `GithubAppAuthError` is the shared
failure type for the whole `github_auth` package.
...
def test_missing_app_id_fails_closed_with_actionable_diagnostic() -> None:
    result = load_github_app_config(environ={"GITHUB_PRIVATE_KEY": _PEM})
...
def test_missing_private_key_fails_closed_naming_the_var() -> None:
    result = load_github_app_config(environ={"GITHUB_APP_ID": "123456"})
    assert not is_successful(result)
    assert "GITHUB_PRIVATE_KEY" in result.failure().detail
