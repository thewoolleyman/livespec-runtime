...
- Every `AttentionKind` member MUST correspond to exactly one stable-ID
  ...
  correspondence MUST be by equal literal text. Adding a kind without
  its prefix, adding a prefix without its kind, or naming a prefix
  differently from its kind is FORBIDDEN. A mechanical check MUST
  assert the bijection so the two vocabularies cannot drift.
- As of the revision that ratifies the constraint above, the
  implementation is NON-CONFORMANT in two places: `internal` is a
  ratified kind with no accepted prefix, and the ratified kind
  `human-valve` is composed with the prefix `valve`. Both are
  spec-to-implementation gaps to be closed in the implementation;
  neither is a licence to weaken the constraint. Closing them MUST
  preserve every id already emitted, so the correction either admits
  `internal` and `human-valve` as accepted prefixes or migrates emitted
  ids under an explicit compatibility decision recorded at revise.
