...
- **Given** the ratified `AttentionKind` members
- **When** the accepted prefix set of `validate_attention_item_id` is
  compared against them
- **Then** the two sets MUST be equal by literal text
...
- **Then** the result MUST surface the invalid candidate as an explicit
  failure or malformed marker, and MUST NOT be a one-element list
  carrying only the valid candidate
