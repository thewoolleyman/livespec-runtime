...
- `branch_exists_on_remote(*, github_url: str, name: str) ->
  ...
  response as `False`. The 404 SHOULD be detected via `gh`'s exit code
  (or a structured response field), not via a substring match on
  stderr; consumers MAY rely on the False success value for any branch
...
```jsonc
{
  "livespec-runtime": {
    "compat": {
      ...
      "pinned": "v0.22.0" // x-release-please-version
    }
  }
}
```
...
[tool.uv.sources]
livespec-runtime = { git = "https://github.com/thewoolleyman/livespec-runtime.git", tag = "v0.22.0" } # x-release-please-version
```
