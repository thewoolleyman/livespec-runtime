...
ratified inventory, is explicitly declared internal there, or is the
one family still recorded as ACKNOWLEDGED DEBT below. The families that
`contracts.md` §"Module-level public surface" ratifies as this
library's SINGLE import path are `livespec_runtime.credentials`,
`livespec_runtime.hygiene_scan`, and `livespec_runtime.spec_governance`.
Where a family's entry point is ratified, its size-decomposition
split-out modules are declared INTERNAL and named as such in that
family's section: consumers MUST import from the ratified entry point,
and this repository MAY change a split-out's surface without a major
classification. A module that ships a public surface and appears in
none of those places is a defect rather than tolerated debt, and
`tests/livespec_runtime/test_public_surface_inventory.py` fails on it;
the `# @generated` third-party port that test's register excludes is
outside this rule by construction.

The `livespec_runtime.github_budget` family (the facade and its four
split-out modules) remains ACKNOWLEDGED DEBT, not a claim that it is
implementation detail. Its ratification is BLOCKED on a constraint
conflict rather than on missing prose: the facade exports
`GithubBudgetedClient`, a deliberately stateful, non-frozen dataclass
holding the conditional-read cache and the mutation-pacing clock, and
`constraints.md` §"Public-surface constraints" requires every public
dataclass to be frozen. Until a subsequent proposal either amends that
constraint with a named exception for the stateful client or disposes
of the client differently, consumers MUST NOT treat the family as
stable API, and this repository MUST NOT rely on the absence as licence
to change it freely where a consumer is known to import it.
