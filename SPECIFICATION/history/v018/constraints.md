...
- Every public error type this library exposes MUST subclass
  `Exception` DIRECTLY. Subclassing a builtin exception subclass —
  `ValueError`, `TypeError`, `RuntimeError`, or any other — is
  FORBIDDEN, and a specification clause mandating one is a defect in
  the clause rather than a licence to violate this rule. Consumers
  MUST catch this library's error types by name, never by a builtin
  ancestor, so the exception hierarchy stays flat and every raise site
  is greppable by type. This specification MUST NOT restate the
  enforcing check's allowlist, which is owned by the enforcement suite
  and MAY widen independently; this rule binds this library's own
  public error types regardless of what that allowlist happens to
  permit.
...
- The composition-completeness constraints above are enforced AT
  CONSTRUCTION: `AttentionItem` validates its own `id` and raises
  `InvalidAttentionItemIdError`, so neither the composer nor a direct
  constructor can produce or suppress an invalid item. Enforcement MUST
  NOT be relocated to the producer emission boundary, because producers
  live in consumer repositories and this library cannot verify a rule it
  does not own.
...
- Every provider MUST surface a transport failure (CalledProcessError,
  JSONDecodeError) on the FAILURE TRACK as a typed `GithubFailure`,
  never by raising to the caller and never by discarding it. The retry
  layer folds that failure track; the resolve layer translates
  exhaustion to `UNKNOWN`. Swallowing a transport failure — returning a
  success value that hides it — remains FORBIDDEN, and is the same
  false-all-clear this specification forbids elsewhere. A raised
  exception is reserved for a CALLER DEFECT, such as the
  non-canonical-URL case below, not for a transport outcome.
- `NonCanonicalGithubUrlError` is the github provider's only domain
  ...
  exceptions but MUST keep them in the provider module and MUST
  subclass `Exception` directly, per §"Public-surface constraints".
  The retry layer's broad catch is `except Exception`, so it already
  catches every such exception; no part of the retry path depends on a
  `ValueError` ancestry.
